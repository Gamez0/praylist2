...
import com.praylist.socialcomponents.main.base.BasePresenter;
import com.praylist.socialcomponents.main.interactors.ProfileInteractor;
        ...
        ifViewAttached(view -> {
            if (result.isSuccess()) {
                ...
                if(code == GoogleSignInStatusCodes.SIGN_IN_REQUIRED){
                    LogUtil.logDebug(TAG, "SIGN_IN_GOOGLE failed : SIGN_IN REQUIRED");
                }
                else if(code == GoogleSignInStatusCodes.NETWORK_ERROR){
                    LogUtil.logDebug(TAG, "SIGN_IN_GOOGLE failed : NETWORK_ERROR");
                }
                else{
                    LogUtil.logDebug(TAG, "SIGN_IN_GOOGLE failed : "+code);
                }
            }
        });
    }
